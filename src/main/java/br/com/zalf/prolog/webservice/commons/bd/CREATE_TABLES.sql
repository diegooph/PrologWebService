...
CREATE TABLE IF NOT EXISTS PRE_CONTRACHEQUE_ITENS (
  ...
  REFERENCES UNIDADE (CODIGO),
  CONSTRAINT FK_PRE_CONTRACHEQUE_ITENS_COLABORADOR FOREIGN KEY (CPF_COLABORADOR)
  REFERENCES COLABORADOR (CPF)
);
...
CREATE TABLE IF NOT EXISTS PNEU_ORDEM (
  ...
  CONSTRAINT FK_PNEU_ORDEM_PNEU_POSICAO FOREIGN KEY (POSICAO_PROLOG) REFERENCES PUBLIC.PNEU_POSICAO(PNEU_POSICAO),
...
CREATE TABLE IF NOT EXISTS PNEU_ORDEM_NOMENCLATURA_UNIDADE (
  ...
  REFERENCES UNIDADE(CODIGO),
  CONSTRAINT FK_PNEU_ORDEM_NOMENCLATURA_UNIDADE_PNEU_POSICAO FOREIGN KEY (POSICAO_PROLOG)
  REFERENCES PNEU_POSICAO (POSICAO_PNEU)
);
...
/**
 * Vincula um código de unidade do ProLog ao código da mesma unidade utilizado no ERP do cliente.
 */
CREATE TABLE IF NOT EXISTS INTEGRACAO_UNIDADE (
  COD_UNIDADE_PROLOG BIGINT NOT NULL,
  COD_UNIDADE_CLIENTE VARCHAR(255) NOT NULL,
  CONSTRAINT PK_INTEGRACAO_UNIDADE PRIMARY KEY (COD_UNIDADE_PROLOG, COD_UNIDADE_CLIENTE),
  CONSTRAINT FK_INTEGRACAO_UNIDADE_UNIDADE FOREIGN KEY (COD_UNIDADE_PROLOG)
  REFERENCES UNIDADE (CODIGO)
);
...
CREATE TABLE IF NOT EXISTS INTERVALO (
  ...
  FONTE_DATA_HORA_INICIO VARCHAR(255),
  DATA_HORA_FIM TIMESTAMP WITHOUT TIME ZONE,
  FONTE_DATA_HORA_FIM VARCHAR(255),
  ...
  UNIQUE(COD_UNIDADE, COD_TIPO_INTERVALO, CPF_COLABORADOR, DATA_HORA_FIM)
);

ALTER TABLE INTERVALO ALTER COLUMN JUSTIFICATIVA_ESTOURO NAME JUSTIFICATIVA
ALTER TABLE INTERVALO ADD COLUMN LATITUDE_INICIO TEXT;
ALTER TABLE INTERVALO ADD COLUMN LONGITUDE_INICIO TEXT;
ALTER TABLE INTERVALO ADD COLUMN LATITUDE_FIM TEXT;
ALTER TABLE INTERVALO ADD COLUMN LONGITUDE_FIM TEXT;

CREATE TABLE IF NOT EXISTS INTERVALO_UNIDADE (
  COD_UNIDADE BIGINT NOT NULL,
  VERSAO_DADOS BIGINT NOT NULL,
  CONSTRAINT FK_INTERVALO_UNIDADE_UNIDADE FOREIGN KEY (COD_UNIDADE)
  REFERENCES UNIDADE(CODIGO),
  CONSTRAINT PK_INTERVALO_UNIDADE PRIMARY KEY (COD_UNIDADE)
);
