	...
	public PlacaServicoHolder getPlacasServico(Long codUnidade) throws SQLException{
		...
		try{
			...
			stmt = conn.prepareStatement("SELECT V.PLACA, MOV.TOTAL_MOVIMENTACAO, CAL.TOTAL_CALIBRAGEM "
					+ "FROM SCH_PNEU.VEICULO V join "
					+ "(SELECT VP.PLACA AS PLACA_MOV, COUNT(ITS.TIPO_SERVICO) AS TOTAL_MOVIMENTACAO "
					+ "FROM SCH_PNEU.VEICULO_PNEU VP "
					+ "JOIN SCH_PNEU.ITEM_SERVICO ITS ON ITS.COD_PNEU = VP.COD_PNEU "
					...
					+ "FROM SCH_PNEU.VEICULO_PNEU VP "
					+ "JOIN SCH_PNEU.ITEM_SERVICO ITS ON ITS.COD_PNEU = VP.COD_PNEU WHERE ITS.TIPO_SERVICO = ? "
	...
	private void setServicos(ServicoHolder holder) throws SQLException{
		...
		try{
			...
			stmt = conn.prepareStatement("SELECT V.PLACA, V.KILOMETRAGEM,V.COD_UNIDADE AS COD_UNIDADE, "
					...
					+ "FROM SCH_PNEU.ITEM_SERVICO ITS "
					+ "JOIN SCH_PNEU.PNEU P ON ITS.COD_PNEU = P.CODIGO "
					+ "JOIN SCH_PNEU.MODELO_PNEU MP ON MP.CODIGO = P.COD_MODELO "
					+ "JOIN SCH_PNEU.MARCA_PNEU MAP ON MAP.CODIGO = MP.COD_MARCA "
					+ "JOIN SCH_PNEU.DIMENSAO_PNEU DP ON DP.CODIGO = P.COD_DIMENSAO "
					+ "JOIN SCH_PNEU.AFERICAO A ON A.CODIGO = ITS.COD_AFERICAO "
					+ "JOIN SCH_PNEU.VEICULO V ON V.PLACA = A.PLACA_VEICULO "
					+ "JOIN SCH_PNEU.VEICULO_PNEU VP ON VP.COD_PNEU = ITS.COD_PNEU "
