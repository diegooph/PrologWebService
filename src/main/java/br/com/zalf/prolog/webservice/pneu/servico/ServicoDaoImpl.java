	...
	public PlacaServicoHolder getPlacasServico(Long codUnidade) throws SQLException{
		...
		try{
			...
			stmt = conn.prepareStatement("SELECT V.PLACA, MOV.TOTAL_MOVIMENTACAO, CAL.TOTAL_CALIBRAGEM, INSP.TOTAL_INSPECAO "  
					...
					+ "FULL OUTER JOIN (SELECT VP.PLACA AS PLACA_MOV, COUNT(ITS.TIPO_SERVICO) AS TOTAL_MOVIMENTACAO "  
					+ "FROM VEICULO_PNEU VP   "
					+ "JOIN AFERICAO_MANUTENCAO ITS ON ITS.COD_PNEU = VP.COD_PNEU WHERE ITS.TIPO_SERVICO = ? AND ITS.DATA_HORA_RESOLUCAO IS NULL "  
					+ "GROUP BY 1,ITS.TIPO_SERVICO) AS MOV ON PLACA_MOV = V.PLACA "  
					+ "FULL OUTER JOIN (SELECT VP.PLACA AS PLACA_CAL, COUNT(ITS.TIPO_SERVICO) AS TOTAL_CALIBRAGEM "  
					+ "FROM VEICULO_PNEU VP "  
					+ "JOIN AFERICAO_MANUTENCAO ITS ON ITS.COD_PNEU = VP.COD_PNEU WHERE ITS.TIPO_SERVICO = ?  AND ITS.DATA_HORA_RESOLUCAO IS NULL "  
					+ "GROUP BY 1,ITS.TIPO_SERVICO) AS CAL ON PLACA_CAL = V.PLACA "  
					+ "FULL OUTER JOIN (SELECT VP.PLACA AS PLACA_INSP, COUNT(ITS.TIPO_SERVICO) AS TOTAL_INSPECAO " 
					+ "FROM VEICULO_PNEU VP "  
					+ "JOIN AFERICAO_MANUTENCAO ITS ON ITS.COD_PNEU = VP.COD_PNEU WHERE ITS.TIPO_SERVICO = ? AND ITS.DATA_HORA_RESOLUCAO IS NULL "  
