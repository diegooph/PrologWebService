...
public interface ChecklistOrdemServicoApiDoc {
    ...
    List<ChecklistOrdemServicoListagemDto> getOrdensServico(
            @ApiParam(value = "Lista de códigos de unidade.",
                      example = "215",
                      required = true)
            @Required final List<Long> codUnidades,
            ...
            @ApiParam(value = "Status da ordem de serviço. Podendo ser A (aberta) ou F (fechada). Utilizado para " +
                    "filtrar ordens de serviço de apenas um veículo. Caso não deseje filtrar, basta não enviar esse " +
                    "parâmetro.",
                      example = "F")
