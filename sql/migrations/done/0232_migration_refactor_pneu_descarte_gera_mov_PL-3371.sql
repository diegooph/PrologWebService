alter table movimentacao_origem
    drop constraint check_movimentacao_origem_tipo_origem,
    add constraint check_movimentacao_origem_tipo_origem
        check (((tipo_origem)::text = 'ESTOQUE'::text) or ((tipo_origem)::text = 'RECAPAGEM'::text) or
               ((tipo_origem)::text = 'EM_USO'::text) or ((tipo_origem)::text = 'ANALISE'::text) or
               ((tipo_origem)::text = 'DESCARTE'));

alter table pneu_retorno_descarte
    drop column if exists cod_processo_movimentacao_descarte,
    drop column if exists cod_movimentacao_descarte,
    drop column if exists status_destino_resultado,
    alter column motivo_retorno_descarte drop not null;
