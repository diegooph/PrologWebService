...
BEGIN
    ...
    THEN
        ...
        SET PLACA_VEICULO_CADASTRO = F_PLACA_NOVA,
            COD_VEICULO_CADASTRO_PROLOG = V_COD_VEICULO_NOVO
    ...
    -- MODIFICA CÓDIGO DO VEÍCULO NO SOCORRO EM ROTA.
    IF EXISTS(SELECT SRA.COD_VEICULO_PROBLEMA
              FROM SOCORRO_ROTA_ABERTURA SRA
              WHERE SRA.COD_VEICULO_PROBLEMA = F_COD_VEICULO_ANTIGO)
    THEN
        UPDATE SOCORRO_ROTA_ABERTURA
        SET COD_VEICULO_PROBLEMA = V_COD_VEICULO_NOVO
        WHERE COD_VEICULO_PROBLEMA = F_COD_VEICULO_ANTIGO;
        GET DIAGNOSTICS V_QTD_UPDATES = ROW_COUNT;
        IF (V_QTD_UPDATES IS NULL OR V_QTD_UPDATES <= 0)
        THEN
            RAISE EXCEPTION
                'Não foi possível modificar a placa para % nos socorros em rotas', F_PLACA_NOVA;
        END IF;
    END IF;

    -- MODIFICA CÓDIGO DO VEÍCULO NAS TRANSFERÊNCIAS.
    IF EXISTS(SELECT VTI.COD_VEICULO
              FROM VEICULO_TRANSFERENCIA_INFORMACOES VTI
              WHERE VTI.COD_VEICULO = F_COD_VEICULO_ANTIGO)
    THEN
        UPDATE VEICULO_TRANSFERENCIA_INFORMACOES
        SET COD_VEICULO = V_COD_VEICULO_NOVO
        WHERE COD_VEICULO = F_COD_VEICULO_ANTIGO;
        GET DIAGNOSTICS V_QTD_UPDATES = ROW_COUNT;
        IF (V_QTD_UPDATES IS NULL OR V_QTD_UPDATES <= 0)
        THEN
            RAISE EXCEPTION
                'Não foi possível modificar a placa para % nas transferências', F_PLACA_NOVA;
        END IF;
    END IF;
