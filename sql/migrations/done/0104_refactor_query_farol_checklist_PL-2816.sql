-- O que é o Farol de checklist?
-- 1 - Mostra todas as placas com itens críticos pendentes no dia e as liberadas (que não possuem itens críticos)
-- 2 - Mostra todas as placas com checklist
-- 3 - Mostra se a placa possui itens críticos pendentes de checklists passados (apenas se habilitado)
-- Regras da funcionalidade?
-- A function deve lidar com um periodo dinâmico de dias de busca.
-- A function deve lidar com uma flag para buscar itens retroativos ou não.
-- A function deve listar todas as placas ativas da unidade.
-- Para placas com itens pendentes, deve mostrar bloqueado (considerando a flag de retroativos).
drop function func_checklist_get_farol_checklist(bigint, date, date, boolean);
...
declare
    ...
    v_qtd_maxima_itens_criticos bigint := 5;
begin
    return query
            ...
            where c.cod_unidade = f_cod_unidade
             ...
             itens_criticos_filtrados as (
                 ...
                 where cosi.cod_unidade = f_cod_unidade
                   ...
                   and f_if(f_itens_criticos_retroativos,
                            true,
                            (c.data_hora_realizacao_tz_aplicado::date between f_data_inicial and f_data_final))
        ...
        from ultimos_checklists_veiculos ucv
                 ...
                 left join itens_criticos_placa icp
                           on f_if(icp.cod_checklist_item in (ucv.cod_checklist_saida, ucv.cod_checklist_retorno),
                                   icp.cod_checklist_item in (ucv.cod_checklist_saida, ucv.cod_checklist_retorno),
                                   icp.placa_veiculo_checklist_item = ucv.placa)
