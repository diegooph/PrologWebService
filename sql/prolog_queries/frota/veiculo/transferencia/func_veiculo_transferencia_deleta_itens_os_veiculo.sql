CREATE OR REPLACE FUNCTION
    FUNC_VEICULO_TRANSFERENCIA_DELETA_ITENS_OS_VEICULO(F_COD_VEICULO BIGINT,
...
BEGIN
    FOR F_OS IN
        SELECT COS.CODIGO_PROLOG,
               ...
               COS.COD_UNIDADE,
               COS.COD_CHECKLIST
               -- Utilizamos a view e não a tabela _DATA, para não copiar OSs deletadas e não resolvidos.
        FROM CHECKLIST_ORDEM_SERVICO COS
                 JOIN CHECKLIST C ON C.CODIGO = COS.cod_checklist
        WHERE COS.STATUS = F_STATUS_OS_ABERTA
          AND C.COD_VEICULO = F_COD_VEICULO
        LOOP
